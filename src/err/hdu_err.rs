...
impl Display for InvalidRecordValueError {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    write!(
      ...
      self.invalid_value, self.keyword, self.allowed_values
    )?;
    Ok(())
  }
}
impl InvalidRecordValueError {
  pub fn new(keyword: &str, invalid_value: &str, allowed_values: &'static [&str]) -> Self {
...
impl Display for NotImplementedErr {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    write!(f, "Error while constructing HDU: extension {} not yet implemented", self.xtnsion)
  }
}
