    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match &self.index.0 {
            Some(col) => write!(f,
                ...
                col, &self.index.1,
                &self.tbl_shape.0.unwrap(), &self.tbl_shape.1
            ), None => write!(f,
...
impl IndexOutOfRangeErr {
    ...
    pub(crate) fn from_idx(index: (Option<usize>, usize), shape: (Option<usize>, usize))
        -> Self 
    {
...
impl Display for TypeMisMatchErr {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f,
            "cannot modify table of type {} with {}",
            self.tbl_type.type_print(), self.wrong_type
        )
    }
}
...
#[derive(Debug, Clone)]
pub struct TblDecodeErr {msg: String}

impl Error for TblDecodeErr {}
impl Display for TblDecodeErr {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {write!(f, "{}", self.msg)}
}
